...
s7:glp_add_cols (lp, size);
  ...
  for (i = 0; i < a1->dims; i++) {
	...
	numfx1_glp = -1 * numfx1_glp;
      ...
      if (i == 2) {
      s11:ia[1 + j + 2*size] = i + 1, ja[1 + j + 2*size] = j + 1, ar[1 + j + 2*size] =
	  numfx1_glp;
      }
...
s12:glp_load_matrix (lp, 3 * size, ia, ja, ar);
s13:glp_simplex (lp, NULL);
s14:status = glp_get_status (lp);
