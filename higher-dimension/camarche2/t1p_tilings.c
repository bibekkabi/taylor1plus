...
for(i=0;i<env->dims;i++) {
 for(p1=env->paf[i]->q;p1;p1=p1->n) {
  ...
  struct zonotope zon;
  zon.row = 3;  /// 3? Dimension 
  zon.column = size;
  zon.totnocols = size;
  zon.nbtiles = 0;
  zon.mat = (double **) calloc (zon.row, sizeof (double *));
  for (i = 0; i < zon.row; i++)
    zon.mat[i] = (double *) calloc (2 * zon.column, sizeof (double));  // 2? Doubtful, will have to check 
  ...
  itv_magnitude (num, env->paf[2]->c);
  double_set_num (&zon.center[2], num);
  if (itv_is_neg (env->paf[0]->c) == 1)
      zon.center[0] = -1 * zon.center[0];
  if (itv_is_neg (env->paf[1]->c) == 1)
      zon.center[1] = -1 * zon.center[1];
  if (itv_is_neg (env->paf[2]->c) == 1)
      zon.center[2] = -1 * zon.center[2];
  ...
  for (j = 0; j < size; j++) {
    ...
    itv_magnitude (num, ptitv2[2][j]);
    double_set_num (&zon.mat[2][j], num);
    if (itv_is_neg (ptitv2[0][j]) == 1)
      zon.mat[0][j] = -1 * zon.mat[0][j];
    if (itv_is_neg (ptitv2[1][j]) == 1)
      zon.mat[1][j] = -1 * zon.mat[1][j];
    if (itv_is_neg (ptitv2[2][j]) == 1)
      zon.mat[2][j] = -1 * zon.mat[2][j];
  }
  /*for(i=0;i<zon.row;i++){
...
// center for first splitted zonotope
    ...
    norm[0] = zon.mat[0][0] * zon.mat[0][0] + zon.mat[1][0] * zon.mat[1][0] + zon.mat[2][0] * zon.mat[2][0];;
    norm[1] = zon.mat[0][1] * zon.mat[0][1] + zon.mat[1][1] * zon.mat[1][1] + zon.mat[2][1] * zon.mat[2][1];
    norm[2] = zon.mat[0][2] * zon.mat[0][2] + zon.mat[1][2] * zon.mat[1][2] + zon.mat[2][2] * zon.mat[2][2];
    ...
    tilings[0].center[2] = zon.center[2] - (zon.mat[2][id]) / 2;
...
// generator for first splitted zonotope
    ...
    if (id == 2)
      tilings[0].mat[0][2] = zon.mat[0][2] / 2;
    else
      tilings[0].mat[0][2] = zon.mat[0][2];
    if (id == 0)
    ...
    if (id == 2)
      tilings[0].mat[1][2] = zon.mat[1][2] / 2;
    else
      tilings[0].mat[1][2] = zon.mat[1][2];
    if (id == 0)
      tilings[0].mat[2][0] = zon.mat[2][0] / 2;
    else
      tilings[0].mat[2][0] = zon.mat[2][0];
    if (id == 1)
      tilings[0].mat[2][1] = zon.mat[2][1] / 2;
    else
      tilings[0].mat[2][1] = zon.mat[2][1];
    if (id == 2)
      tilings[0].mat[2][2] = zon.mat[2][2] / 2;
    else
      tilings[0].mat[2][2] = zon.mat[2][2];
...
// center for second splitted zonotope
    ...
    tilings[1].center[2] = zon.center[2] + (zon.mat[2][id]) / 2;
    //printf("%f \t", p2[n].center[k]);}
...
// generator for second splitted zonotope
    ...
    tilings[1].mat[0][2] = tilings[0].mat[0][2];
    tilings[1].mat[1][0] = tilings[0].mat[1][0];
    tilings[1].mat[1][1] = tilings[0].mat[1][1];
    tilings[1].mat[1][2] = tilings[0].mat[1][2];
    tilings[1].mat[2][0] = tilings[0].mat[2][0];
    tilings[1].mat[2][1] = tilings[0].mat[2][1];
    tilings[1].mat[2][2] = tilings[0].mat[2][2];
    
    /*for(i=0;i<tilings[0].row;i++){
    for(j=0;j<tilings[0].column;j++){
        printf("%f \t",tilings[0].mat[i][j]);}
    ...
    s.n = 2;
    s.abs = (ap_abstract0_t **) calloc (2, sizeof (ap_abstract0_t *));  /// 2? s.n = 2; no: of tiles

    for (i = 0; i < tilings[0].nbtiles; i++) {
      t1p_t *tiles0 = t1p_alloc (man, 0, 3);
	...
	t1p_aff_build (pr1, tiles0->paf[1], coeff, pr1->dim - 1);
      ...
      tiles0->paf[2] = t1p_aff_alloc_init (pr1);
      num_set_double (num, tilings[i].center[2]);
      itv_set_num (coeff, num);
      itv_set (tiles0->paf[2]->c, coeff);
      num_set_double (num, tilings[i].mat[2][0]);
      itv_set_num (coeff, num);
      if (itv_is_eq (coeff, coeffchck) == 0) {
	t1p_aff_build (pr1, tiles0->paf[2], coeff, pr1->dim - 1);
      }
	...
	t1p_aff_build (pr1, tiles0->paf[1], coeff, pr1->dim - 1);
      ...
      num_set_double (num, tilings[i].mat[2][1]);
      itv_set_num (coeff, num);
      if (itv_is_eq (coeff, coeffchck) == 0) {
	t1p_aff_build (pr1, tiles0->paf[2], coeff, pr1->dim - 1);
      }
      
      num_set_double (num, tilings[i].mat[0][2]);
      itv_set_num (coeff, num);
      t1p_nsym_t *symb3 = t1p_nsym_add (pr1, UN);
      t1p_aff_nsym_add (pr1, tiles0->paf[0], coeff, symb3);
      num_set_double (num, tilings[i].mat[1][2]);
      itv_set_num (coeff, num);
      if (itv_is_eq (coeff, coeffchck) == 0) {
	t1p_aff_build (pr1, tiles0->paf[1], coeff, pr1->dim - 1);
      }
      num_set_double (num, tilings[i].mat[2][2]);
      itv_set_num (coeff, num);
      if (itv_is_eq (coeff, coeffchck) == 0) {
	t1p_aff_build (pr1, tiles0->paf[2], coeff, pr1->dim - 1);
      }
     ...
     /*#ifdef _T1P_DEBUG
    t1p_fprint(stdout, man, tiles0, 0x0);
    fprintf(stdout, "### ### ###\n");
  #endif
  scanf("%f",&b);*/
