...
static inline void t1p_aff_exchange_coeff(t1p_internal_t *pr, t1p_aff_t* expr, uint_t index, itv_t newcoeff);

static inline void t1p_aff_update(t1p_internal_t *pr, t1p_t* a);

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/* returns the generator matrix */
static inline void t1p_aff_get_gen(t1p_internal_t *prbx, t1p_t* tb,itv_t** ptitv[tb->dims][prbx->dim]);

static inline void t1p_aff_get_gen_num(uint_t size, t1p_t* tb, itv_t** ptitv[tb->dims][size], num_t a);

static inline void t1p_aff_get_gen_num_size(t1p_internal_t *prbx, t1p_t* tb, itv_t** ptitv[tb->dims][prbx->dim], num_t a);

static inline void t1p_aff_get_gen_size(uint_t size, t1p_t* tb, itv_t** ptitv[tb->dims][size]);

static inline uint_t t1p_aff_get_generator(t1p_internal_t *pr, t1p_t* env, itv_t** ptitv[env->dims][pr->dim], num_t a);

/* returns the generator matrix in a more efficient way*/
static inline uint_t t1p_aff_get_generator_matrix_size(t1p_internal_t *pr, t1p_t* env, itv_t** ptitv);

static inline void t1p_aff_get_generator_matrix(t1p_internal_t *pr, t1p_t* env, itv_t** ptitv, itv_t** ptitv1);

static inline void t1p_aff_get_generator_matrix_num(t1p_internal_t *pr, t1p_t* env, itv_t** ptitv, itv_t** ptitv1, num_t a);
...
static inline void t1p_aff_update(t1p_internal_t *pr, t1p_t* a)
{
  ...
  size_t size1 = t1p_nsymcons_get_dimension(pr, a);
    //char** name_of_ns = (char**)malloc(size1*sizeof(char*));
    ap_dim_t dim = 0;
   itv_t mid, dev;
   itv_t bound_itv, cen, cen1; 
    itv_init(mid); itv_init(dev); itv_init(bound_itv); itv_init(cen), itv_init(cen1);

  for (l=0; l<size1; l++) {
	//name_of_ns[l] = (char*)malloc(10*sizeof(char));
	//pr->epsilon[a->nsymcons[l]]->type == IN ? sprintf(name_of_ns[l], "eps%d", a->nsymcons[l]) : sprintf(name_of_ns[l], "eta%d", a->nsymcons[l]);
        //printf("eta%d", a->nsymcons[l]);
        bound = ap_abstract0_bound_dimension(pr->manNS, a->abs, l);
        itv_set_ap_interval(pr->itv, bound_itv, bound);
        itv_middev(pr->itv, mid, dev, bound_itv);
        //printf("%zu\n", l);
        if (t1p_aff_get_coeff(pr, a->paf[0], a->nsymcons[l]!=NULL)){
        //itv_print(t1p_aff_get_coeff(pr, a->paf[1], a->nsymcons[l]));
        //printf("%f", bound->inf->val.dbl);   
        itv_mul(pr->itv, cen, mid, t1p_aff_get_coeff(pr, a->paf[0], a->nsymcons[l]));
        itv_mul(pr->itv, cen1, dev, t1p_aff_get_coeff(pr, a->paf[0], a->nsymcons[l]));
        t1p_aff_exchange_coeff(pr, a->paf[0], a->nsymcons[l], cen1);
        itv_add(a->paf[0]->c, a->paf[0]->c, cen); }
        if (t1p_aff_get_coeff(pr, a->paf[1], a->nsymcons[l])!=NULL){
        itv_mul(pr->itv, cen, mid, t1p_aff_get_coeff(pr, a->paf[1], a->nsymcons[l]));
        itv_mul(pr->itv, cen1, dev, t1p_aff_get_coeff(pr, a->paf[1], a->nsymcons[l]));
        t1p_aff_exchange_coeff(pr, a->paf[1], a->nsymcons[l], cen1);
        itv_add(a->paf[1]->c, a->paf[1]->c, cen);}
        //itv_print(a->paf[0]->c);
        //itv_print(a->paf[1]->c);
        //ap_interval_set_double(a->gamma[l], -1, 1);
        //ap_interval_fprint(stdout, a->gamma[l]);
        //scanf("%f",&b);
    }
...
for (i=0;i<tb->dims;i++){
   ...
   for (j=0;j<size;j++){
       if (cell != NULL){
          itv_mul_num(cell->coeff, cell->coeff, a);
          ptitv[i][cell->pnsym->index]=cell->coeff;
          //itv_print(ptitv[i][cell->pnsym->index]);
          cell = cell->n;
      }
     }
}
...
int count=0;
   ...
   for (j=0;j<pr->dim-count;j++){
      if (ptitv[0][j]==coeff1 && ptitv[1][j]==coeff1){
          count++;
          printf("%u", j);
          for (i=j;i<pr->dim-count;i++){
          ptitv[0][i]=ptitv[0][i+1];
          ptitv[1][i]=ptitv[1][i+1];}
          }
}
...
///////////// new efficient method to compute generator matrix ///////////////////////////////
...
for(i=0;i<env->dims;i++) {
 for(p1=env->paf[i]->q;p1;p1=p1->n) {
		itv_init(ptitv1[i][vec[p1->pnsym->index]]);
                itv_init(tmp);
                itv_mul_num(tmp, p1->coeff, a);
		itv_set(ptitv1[i][vec[p1->pnsym->index]],tmp);
                //itv_print(ptitv1[i][p1->pnsym->index]);
