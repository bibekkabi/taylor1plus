...
# Object files required for tilings
#------------------------------------
all: horzcat.o stack.o genhorzcat.o vertcat.o find.o dichoto.o circshift1.o signsvec.o signsvecnew.o
#---------------------------------------
...
CCMODULES = t1p_internal t1p_representation t1p_constructor t1p_meetjoin t1p_assign t1p_resize t1p_otherops t1p_fun t1p_itv_utils horzcat stack genhorzcat vertcat dichoto find circshift1 t1p_tilings signsvec signsvecnew
...
LIBS = -lapron -lgmpxx -lgmp -lmpfr -lglpk -llapack -lblas -lm
LIBS_DEBUG = -lapron_debug -lgmpxx -lgmp -lmpfr -lglpk -lm
...
#testD testMPQ testMPFR testD_debug testMPQ_debug testMPFR_debug 

#test_joinD_debug test_joinMPQ_debug test_joinMPFR_debug 
...
#	$(CC) $(CFLAGS) -DNUM_DOUBLE -o $@ t1p_internal$*.o t1p_representation$*.o t1p_tilings$*.o horzcat.o stack.o genhorzcat.o vertcat.o signsvec_new_while.o dichoto.o find.o circshift1.o $(LIBS) -L. $< -lt1p$* -lbox$* -lpolkaMPQ_debug -lapron

#test%_debug: t1p_test_eval_texp%_debug.o
#	$(CC) $(CFLAGS_DEBUG) -DNUM_DOUBLE -o $@ t1p_internal$*_debug.o t1p_tilings$*_debug.o horzcat.o stack.o genhorzcat.o vertcat.o signsvec_new_while.o dichoto.o find.o circshift1.o $(LIBS_DEBUG) -L. $< -lt1p$*_debug -lbox$*_debug -lpolkaMPQ_debug 

#test_join%: t1p_test_join%.o
#	$(CC) $(CFLAGS_DEBUG) -DNUM_DOUBLE -o $@ t1p_internal$*.o t1p_representation$*.o $(LIBS) -L. $< -lt1p$* -lbox$* -lpolkaMPQ_debug
...
dist: $(CCSRC) Makefile perlscript_caml.pl perlscript_c.pl macros.pl t1p.idl t1p.ml t1p.mli t1p_caml.c
...
t1p%.cmxa t1p%.a: t1p.cmx libt1p%_caml.a libt1p%.a
	$(OCAMLMKLIB) -o t1p$* -oc t1p$*_caml_debug t1p.cmx -lt1p$*_debug $(LIBS_DEBUG)
#t1p%.cmxa t1p%.a: t1p.cmx libt1p%_caml.a libt1p%.a
...
%.ml %.mli %_caml.c: %.idl perlscript_caml.pl perlscript_c.pl macros.pl ../mlapronidl/manager.idl
	mkdir -p tmp
	cp $*.idl ../mlapronidl/*.idl tmp/
	cd tmp && $(CAMLIDL) -no-include -prepro "$(PERL) ../macros.pl" $*.idl
